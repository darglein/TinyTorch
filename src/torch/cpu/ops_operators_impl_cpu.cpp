...
void mult_backward_impl_cpu(Tensor a, double b, Tensor grad_output, Tensor& grad_a)
{
...
template <typename T>
static void div_backward_impl_cpu( double a, TensorInfo<T> b, TensorInfo<T> grad_output, TensorInfo<T> grad_a)
{
    for (int64_t i = 0; i < b.numel(); ++i)
    {
        auto g        = grad_output[i];
...
void div_backward_impl_cpu(double a, Tensor b , Tensor grad_output, Tensor& grad_a)
{
    SWITCH_MACRO_ALL(b.scalar_type(), div_backward_impl_cpu, a, b, grad_output, grad_a);
}
